...
class Wechat
{
    ...
    // 小程序模板消息接口
    const WXSS_TEMPLATE_TITLE_LIST_URL = '/wxopen/template/library/list?';
    const WXSS_TEMPLATE_KEYWORD_LIST_URL = '/wxopen/template/library/get?';
    const WXSS_TEMPLATE_ADD_TPL_URL = '/wxopen/template/add?';
    const WXSS_TEMPLATE_LIST_URL = '/wxopen/template/list?';
    const WXSS_TEMPLATE_DEL_TPL_URL = '/wxopen/template/del?';
    const WXSS_TEMPLATE_SEND_URL = '/message/wxopen/template/send?';
    ...
    {
        $code = empty($code) ? (isset($_GET['code']) ? $_GET['code'] : '') : $code;
        if (empty($code)) {
            return false;
        }
    ...
    public function getBizdecryptData($encryptedData, $iv, $session_key = '')
    {
        ...
        if (strlen($session_key) != 24) {
            $this->errCode = ErrorCode::$Biz_IllegalAesKey;
            $this->errMsg = 'encodingAesKey invalid';
            return false;
        }
        $aesKey = base64_decode($session_key);
        if (strlen($iv) != 24) {
            $this->errCode = ErrorCode::$Biz_IllegalIv;
        ...
        $json = json_decode($result, true);
        if (!$json) {
            $this->errCode = ErrorCode::$Biz_IllegalBuffer;
    ...
    public function getTemplateTitleList($data)
    {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . self::WXSS_TEMPLATE_TITLE_LIST_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getTemplateKeywordList($data)
    {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . self::WXSS_TEMPLATE_KEYWORD_LIST_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function setTemplateAddTpl($data)
    {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . self::WXSS_TEMPLATE_ADD_TPL_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getTemplateList($data)
    {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . self::WXSS_TEMPLATE_LIST_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function setTemplateDelTpl($data)
    {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . self::WXSS_TEMPLATE_DEL_TPL_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function sendTemplateMsg($data)
    {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . self::WXSS_TEMPLATE_SEND_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
...
class ErrorCode
{
    ...
    public static $Biz_IllegalAesKey = '-41001';
    public static $Biz_IllegalIv = '-41002';
    public static $Biz_Illegal = '-41003';
    public static $Biz_IllegalBuffer = '-41004';
    public static $Biz_EncodeBase64Error = '-41005';
    public static $Biz_DecodeBase64Error = '-41006';
